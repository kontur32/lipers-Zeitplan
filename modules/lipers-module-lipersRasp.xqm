...
declare function lipersRasp:класс-урок-день2( $расписаниеДанные, $словарьПредметов ){
...
declare function lipersRasp:класс-урок-день( $расписаниеДанные, $словарьПредметов ){
  ...
  for $i in $расписаниеДанные/row/cell[ position() >= 3 ]
 
  let $класс := tokenize( tokenize( $i, ';')[ 1 ], ',' )
  
  where $класс 
  order by  number( $класс ) descending
  group by $класс
   ...
   return
    <tr>
      ...
      <td>{ $j } урок</td>
      {
        for $d in 1 to 5
        let $предметы := $i[
         number( tokenize( @label, '-')[2] ) = $j and
         number( tokenize( @label, '-')[1] ) = $d ]/text()
        let $названиеПредмета := 
          if( count( $предметы ) = 2 and string-join( $предметы/tokenize( ., ';' )[ 3 ] ) = '////'  or  count( $предметы ) <= 1 )
          then(
            for $предмет in $предметы
        ...
        return
          <td день = '{ $d }'>{ string-join( $названиеПредмета, '/' ) }</td>
      }
    </tr>  
};
...
declare function lipersRasp:рендерингРасписаниеДетское( $расписаниеДанные, $словарьПредметов ){
  <div>
    ...
    <table border = '1px'>
      ...
      { lipersRasp:класс-урок-день2( $расписаниеДанные, $словарьПредметов ) }
    </table>
  </div>
};